...
export const ProjectJob = ({ project }) => {
  ...
  const { data } = useFetch(
    `http://fhunt-env.eba-pr2amuxm.ap-southeast-1.elasticbeanstalk.com/api/v1/job/?projectId=${param.id}`
  );
  useEffect(() => {
    if (data?.data) {
      setJobs(data?.data?.jobDTOList);
      console.log("Job: ", data?.data?.jobDTOList);
    }
  }, [data]);
  return (
